...
if debugPrints
    %% Note, you'll have to update or pass in marginalization names
    if ~isempty(options.marginalizationNames)
        margNames = options.marginalizationNames;
    else
        margNames = sprintfc('Marg %d', 1:ndims(X)-1);
    end
    margNames = [{'Neurons'} margNames(:)'];
    PrintMargSummary(margNames, options);
end
...
for subs = 1:length(paramsubsets)
    ...
    if debugPrints
        fprintf('(%d) Marginalizing over:  [%s] X dims: [%s]\n', subs, PrintMargNames(margNames(sort(indMarg))), num2str(sort(indMarg)))
    end
    ...
    for s = 1:length(paramsubsubsets)-1
        ...
        if debugPrints
            fprintf('    For marg (%d) (dims %s) - subtracting data marginalized over dims %s [%s]\n', ...
                subs, num2str(sort(indMarg)), num2str(sort(indM)), PrintMargNames(margNames(sort(indM))));
        end
    end
    if options.ignoreNaNs
        YY{subs}(isnan(YY{subs})) = 0;
    end
end
%%
...
function str = PrintMargNames(margNames)
%%
...
for ii = 1:length(margNames)-1
    str = sprintf('%s%s, ',str,margNames{ii});
end
if isempty(ii)
    ii=0;
end
str = sprintf('%s%s',str,margNames{ii+1});




function PrintMargSummary(margNames, options)
fprintf('\nMarginalizing over: ')
fprintf('%s, ', margNames{2:end})
fprintf('\b\b\n')
fprintf('Combining\n')
for ii = 1:length(options.combinedParams)

    fprintf('%d) ', ii)
    printStr = '';
    for jj = 1:length(options.combinedParams{ii})

        for kk = 1:length(options.combinedParams{ii}{jj})
            printName = margNames{options.combinedParams{ii}{jj}(kk)+1};
            if kk == 1
                printStr = sprintf('%s[%s',printStr, printName);
            else
                printStr = sprintf('%s, %s', printStr, printName);
            end
            if kk == length(options.combinedParams{ii}{jj})
                printStr = [printStr ']'];
            end
        end
        if jj ~= length(options.combinedParams{ii})
            printStr = sprintf('%s and ', printStr);
        end
    end
    fprintf('%s\n', printStr)
end
