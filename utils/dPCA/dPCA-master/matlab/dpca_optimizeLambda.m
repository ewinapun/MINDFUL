...
%     CnoiseTrain = diag(sum(bsxfun(@times, SSnoiseSumOverT(:,:), 1./(numOfTrials(:,:)-1)),2));
    ...
    for lmbdI = 1:length(options.lambdas)
        if options.print
            fprintf('.')
        end
        ...
        [W,V,whichMarg] = dpca(Xtrain, options.numComps, ...
            ...
            'lambda', options.lambdas(lmbdI), 'Cnoise', CnoiseTrain);
        ...
        for i=1:length(XtestMargs)
            ...
            errorsMarg(i, lmbdI, rep) = recError/margVar_toNormalize(i);
            cumError = cumError + recError;
        end
        
        errors(lmbdI,rep) = cumError / sum(margVar_toNormalize);
    end
