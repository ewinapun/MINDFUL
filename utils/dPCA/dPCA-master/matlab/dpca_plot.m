function [out, myFig] = dpca_plot(Xfull, W, V, plotFunction, varargin)
...
% 
%   'conditionNames'
%                           - Condition names for the non-stim
%                           (directional) components in the legend
% 
...
X = Xfull(:,:)';
if options.plotMeanSubData
    XtoProj = bsxfun(@minus, X, nanmean(X));
else
    XtoProj = X;
end
...
if ~isempty(options.whichMarg) && ...
   ...
   && ~isempty(options.marginalizationNames)
    ...
    for m = intersect(1:4, unique(options.whichMarg(componentsToPlot)))        
        ...
        a = annotation('rectangle', [offsetX-0.01 pos(2) 0.015 pos(4)], ...
            'EdgeColor', 'none', 'FaceColor', options.marginalizationColours(m,:));
        
%         margNameLen = length(options.marginalizationNames{m});
%         if margNameLen > 15
%             yLoc = pos(2) + pos(4) - 0.001;
%         else
%             yLoc = pos(2) + pos(4)/2 + th.Extent(end)/2;
%         end
        %% Center marg name
%         try; delete(th); end
        th = text(0,0,options.marginalizationNames{m});
        th.FontWeight = 'bold';
        th.Units = 'pixels';
        thWidthPix = th.Extent(3);
        delete(th);
        ax = gca;
        ax.Units = 'pixels';
        axPosPix = ax.Position(4);
        ax.Units = 'normalized';
        txtAxRatio = (thWidthPix/axPosPix);
        yoffset = pos(4)*txtAxRatio;
        if txtAxRatio < 1
            yoffset = yoffset + (pos(4) - yoffset)/2; % Center
        end
        
        %%
%         delete(txtLoc)
        yLoc = pos(2) + yoffset;
        txtLoc = annotation('textarrow', offsetX*[1 1], yLoc*[1 1], ...
            'string', options.marginalizationNames{m}, ...
            'HeadStyle', 'none', 'LineStyle', 'none', ...
            'TextRotation', 90, 'FontWeight', 'bold');
...
if ~isempty(options.titleStr)
    if ~contains(options.titleStr, {newline})
        myFig.Name = options.titleStr;
    end
    an_h = annotation(myFig,'textbox', 'String', options.titleStr, 'HorizontalAlignment', 'center');
    an_h.LineStyle = 'none';
    an_h.Tag = 'dpcaTitle';
    an_h.FontSize = 20;
    an_h.Position = [0.1, 0.88, 0.1, 0.1];
end
%%
...
%     axis([0 numCompToShow+1 0 12.5])
    ...
    for i = 1:size( options.explainedVar.margVar,1)
        b(i).FaceColor = 'flat';
        b(i).CData = options.marginalizationColours(i,:);
    end
    axBar.XLim = [0 numCompToShow+1];
    axBar.YLim(1) = 0;
...
if isempty(find(subplots==1, 1))
    delete(ax)
end

%%
cb = colorbar(axColormap);
...
nSideSubplots = 3;
betweenSummaryBuffer = 0.03;
figWH = fh.Position(3:4);
figWH = figWH(1)/figWH(2);
xStart = 0.05;
% width = 0.14;
% height = width*figWH;

height = ((top-bottom) - betweenSummaryBuffer*(nSideSubplots-1))/nSideSubplots;
% height = (1-2*bottom - betweenSummaryBuffer*(nSideSubplots-1))/nSideSubplots;
width = height/figWH;
xStart = max(xStart, (0.3 - width)/2);
newPos = [xStart, bottom, width, height];
set(axColormap, 'Position', newPos)
...
if ~isempty(options.explainedVar)
    pos = get(axColormap, 'Position');
    bottom = pos(2)+pos(4);
    pos(2) = bottom + betweenSummaryBuffer;
    set(axBar, 'Position', pos)
    
    pos = get(axBar, 'Position');
    bottom = pos(2)+pos(4);
    pos(2) = bottom + betweenSummaryBuffer;
    set(axCum, 'Position', pos)
...
if ~isempty(options.legendSubplot)
    ...
    if ~isempty(options.X_extra)
        plotFunction('legend', size(options.X_extra), options)
    else
...
if ~isempty(options.explainedVar)
    ...
    piW = pos(4)/3; 
    piH = piW*figWH;
    piAxPos = [pos(1)+pos(3)-piW pos(2)+pos(4)-piH*1.22, piW piH];
    piAx = axes('position', piAxPos); % [0.205 0.47 0.1 0.1])
    piAx.XColor = 'none';
    piAx.YColor = 'none';
    piAx.Color = 'none';
